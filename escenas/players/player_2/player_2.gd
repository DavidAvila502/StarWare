class_name player2
extends Node2D
signal update_p2_lives_counter
signal stop_timers
signal update_shots_bar_p2
signal stop_reload_shots_p2
signal p2_die
...
func _on_update_lives_counter():
	emit_signal('update_p2_lives_counter')

func _on_stop_timers():
	emit_signal('stop_timers',1)
	$entry_controller.queue_free()

func _on_update_shots_bar(reload_shot):
	emit_signal('update_shots_bar_p2',reload_shot)
	
func _on_stop_reload_shots():
	emit_signal('stop_reload_shots_p2')
